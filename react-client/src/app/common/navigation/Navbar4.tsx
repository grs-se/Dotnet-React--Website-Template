...
export default function Navbar3() {
	const links = [
		...
		{ text: 'Services', url: '/services', children: [{ text: 'Conservation', url: '/services/conservation' }, { text: 'Conservation', url: '/services/conservation' }] },
						...
						{links.map(link => (
							link.children ? (
								<li className="relative">
									<button onMouseOver={handleDropDown} id="dropdownNavbarLink" className="flex items-center justify-between w-full py-2 pl-3 pr-4 text-gray-900 hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-green-700 md:p-0 md:w-auto dark:text-white md:dark:hover:text-green-500 dark:focus:text-white dark:border-gray-700 dark:hover:bg-gray-700 md:dark:hover:bg-transparent">{link.text} <svg className="w-2.5 h-2.5 ml-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
										<path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4" />
									</svg>
										{/*<!-- Dropdown menu -->*/}
									</button>
									<li>
										<div id="dropdown" onMouseOut={handleDropDown}
